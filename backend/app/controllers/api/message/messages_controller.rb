class Api::Message::MessagesController < ApplicationController
  def index
    my_id = params[:id]
    ...
    rooms.each do |room|
      ...
      messages =
        Message
          .where(user_id: [user_id, my_id], room_id: room_id)
    ...
    ActiveRecord::Base.transaction do
      begin
        ...
        if Room.order(created_at: :desc).pluck(:room_id).first
          new_value =
            Room.order(created_at: :desc).pluck(:room_id).first.to_i + 1
        else
          new_value = 1
        end
