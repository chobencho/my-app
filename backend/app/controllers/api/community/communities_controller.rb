class Api::Community::CommunitiesController < ApplicationController
  def index
    @community =
      Community.joins(:community_category).select(
        "communities.*, community_categories.community_code"
      )
  ...
  def show
    ...
    myCommunity =
      Community
        .joins(:community_users, :community_category)
        .select(
          "*, community_users.community_id AS id, community_users.user_id AS user_id, community_categories.community_code"
        )
        .where(community_users: { user_id: params[:id] })
    latestCommunity =
      Community
    ...
    latest_community_ids =
      CommunityComment.order(updated_at: :desc).pluck(:community_id)
    uniq_latest_community_ids = latest_community_ids.uniq.take(3)
    popularCommunity =
      Community
        .joins(:community_category)
        .select("communities.*, community_categories.community_code")
        .where(id: uniq_latest_community_ids)
